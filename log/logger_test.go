...
import (
	"context"
	"fmt"
	context2 "github.com/exluap/kit/context"
	"github.com/exluap/kit/er"
	"testing"
)

func Test_Clogger_WithCtx(t *testing.T) {
	logger := Init(&Config{Level: TraceLevel})
	ctx := context2.NewRequestCtx().Rest().WithNewRequestId().WithUser("1", "john").ToContext(context.Background())
	l := L(logger).C(ctx)
	l.Inf("I'm logger")
}
...
func Test_Clogger_All(t *testing.T) {
	logger := Init(&Config{Level: TraceLevel})
	ctx := context2.NewRequestCtx().Rest().WithNewRequestId().WithUser("1", "john").ToContext(context.Background())
	l := L(logger).Cmp("service").Mth("do").C(ctx).F(FF{"field": "value"})
	l.Inf("I'm logger")
}

func Test_Clogger_WithFields(t *testing.T) {
	logger := Init(&Config{Level: TraceLevel})
	l := L(logger).F(FF{"field": "value"})
	l.Inf("I'm logger")
}
...
func Test_Clogger_WithAppErr(t *testing.T) {
	logger := Init(&Config{Level: TraceLevel})
	l := L(logger).E(er.New("ERR-123", "%s happened", "shit"))
	l.Err("my bad")
}

func Test_Clogger_WithAppErrAndStack(t *testing.T) {
	logger := Init(&Config{Level: TraceLevel})
	l := L(logger).E(er.New("ERR-123", "%s happened", "shit")).St()
	l.Err("my bad")
}
...
func Test_Clogger_WithAppErrAndFields(t *testing.T) {
	logger := Init(&Config{Level: TraceLevel})
	e := er.WithBuilder("ERR-123", "%s happened", "shit").F(er.FF{"f": "v"}).Err()
	l := L(logger).E(e)
	l.Err("my bad")
}
...
func Test_Clogger_WithAppErrAndAppContext(t *testing.T) {
	logger := Init(&Config{Level: TraceLevel})
	ctx := context2.NewRequestCtx().Test().WithRequestId("123").ToContext(context.Background())
	e := er.WithBuilder("ERR-123", "%s happened", "shit").C(ctx).Err()
	l := L(logger).E(e)
	l.Err("my bad")
}
